...
const Index = () => {
  ...
  const calculateTotalPrice = (items: BuildComponent[]): number => {
    return items.reduce((sum, component) => {
      let price: number = 0;
      if (typeof component.price === "string") {
        const parsed = parseFloat(
          (component.price as string).replace(/[^0-9.-]+/g, "")
        );
        price = isNaN(parsed) ? 0 : parsed;
      } else if (typeof component.price === "number") {
  ...
  const handleBackToOptions = () => {
    ...
    if (view === "build") {
      // If in build view, go back to chat view (do not reset chat)
      setView("chat");
      return;
    }
    // If in chat view, go back to mode selection (main screen)
    setMode(null);
    setView("chat");
    setMessages([
      {
      ...
      <div className="w-full max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto text-lg">
            Select your budget and let our AI create the perfect build for your
            needs
          </p>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(Object.keys(componentData) as ComponentType[]).map((type) => {
                ...
                return (
                  <div
                    key={type}
                    className={`group relative overflow-hidden rounded-2xl p-5 transition-all duration-300 cursor-pointer border border-gray-700/50 hover:border-transparent ${
                      component.gradient
                  ...
                  >
                    <div className="flex flex-col h-full">
                      <div className="flex items-start justify-between mb-4">
                        <div
                          className={`p-3 rounded-xl bg-gradient-to-br ${component.color}/20`}
                        >
                          <div
                            className={`text-${component.color.split(" ")[0]}`}
                          >
                            {component.icon}
                          </div>
                        </div>
                      ...
                      <div className="flex flex-col items-center text-center">
                        ...
                        <div className="mt-2 w-full h-1.5 bg-gray-600 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (budget / 2500) * 100)}%`,
                            }}
                          />
                        </div>
                      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6">
              ...
              <p className="text-sm text-gray-400 mb-4">
                Our AI assistant can help you choose the right components for
                your needs.
              </p>
              <Button
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const buildMatch = input.match(
        /(build|create).*?(\$?\d{3,4}|\d{3,4}\s*\$)/i
      );
      ...
      if (response.type === "build") {
        ...
        const componentsList = Array.isArray(response.data.components)
          ...
          : Object.entries(response.data.components || {}).map(
              ([type, details]: [string, BuildComponent]) => ({
                ...details,
                type,
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          text:
            response.content ||
            (isBuildRequest ? "Here's your PC build!" : "Here's your response"),
          isBot: true,
        },
      ]);
    } catch (error) {
  ...
  const handleBuildPC = useCallback(
    async (budget?: number) => {
      if (!mode) {
        setIsLoading(false);
        return;
      ...
      try {
        const response: ChatResponseData = await chatService.sendMessage({
          message: `Build me a ${buildType} PC${
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            text:
              response.content ||
              `Here's your ${buildType} PC build${budgetMessage}!`,
            isBot: true,
          },
        ]);
      } catch (error) {
  ...
  const switchToMode = useCallback((newMode: string) => {
    ...
    setMessages([{ id: Date.now(), text: welcomeMessage, isBot: true }]);
  }, []);
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center p-6">
        <div className="text-center mb-12 max-w-4xl mx-auto">
          ...
          <p className="text-xl text-gray-400 max-w-2xl mx-auto mb-12">
            Get expert advice or let our AI create a custom build tailored to
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <p className="text-gray-400 mb-6">
                  Get expert advice and answers to all your PC component
                  questions
                </p>
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <p className="text-gray-400 mb-6">
                  Get a custom PC build recommendation based on your needs and
                  budget
                </p>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="text-center my-8">
              ...
              <p className="text-lg text-gray-400">
                {buildType.charAt(0).toUpperCase() + buildType.slice(1)} Build -
                Total:{" "}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
              {components.map(
                (
                  component: BuildComponent & { icon?: React.ReactNode },
                  index: number
                ) => (
                  <div
                  ...
                  >
                    ...
                    <ul className="space-y-2 text-sm text-gray-400">
                      {component.specs && Array.isArray(component.specs)
              ...
              <Button
                onClick={() => {
                  navigator.clipboard.writeText(
                    `PC Build Total: $${totalPrice.toFixed(2)}`
                  );
                  setShowToast(true);
                  ...
                  setMessages((prev) => [
                    ...
                    {
                      id: Date.now(),
                      text: `Okay, proceeding with the ${buildType} build totaling $${totalPrice.toFixed(
                        2
